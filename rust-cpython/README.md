...
python3 rust-cpython/build.py build --variant base64-lean --base64-lean # optional macOS arm64 public Base64 route
...
`--base64-lean` is restricted to native macOS arm64 and links the pinned-nightly
`no_std` AArch64 encoder into the installed `binascii` extension. It uses
the existing C wrapper for argument parsing and for inputs outside the large,
contiguous, default-alphabet route. The default build and Linux x86-64 build
do not select this patch. The linked archive and exact generated Makefile rule
are recorded in `results/build-base64-lean.json`; see
[`experiments/base64-lean-installed-20260925.md`](experiments/base64-lean-installed-20260925.md)
for the installed qualification evidence.
...
| 18 | **base64 / binascii** — `Lib/base64.py`, `Modules/binascii.c`, `Modules/_base64/`; `test_base64.py`, `test_binascii.py` | Consider raw buffer codec kernels, especially operations not already handled efficiently by `binascii`. | Standard Base64 already enters C; `_base64` is not wired into the public module. The first size sweep in [`PERFORMANCE.md`](PERFORMANCE.md) found Rust 39.5% faster at 64 bytes but 42–48% slower at 4 KiB and above. A guarded public small-input route lost 10–20% CPU per call and 6.7% in a complete catalog diagnostic under host load; reject that route. The optional AArch64 NEON public route matched checked outputs and used about 35% of same-source C encode time at 65 KiB and 1 MiB under host load. Its first standard Rust link added 1.42 MB and 1.6–2.0 MB fresh-process peak RSS. The `no_std` link cut the standalone extension to 91,808 bytes, only 1,760 above C, with fresh-process RSS differences inside control variation. The [lean route](experiments/base64-lean-route-20260925.md) now [builds and imports](experiments/base64-lean-installed-20260925.md) as an optional installed macOS arm64 variant with matching checked output. Complete-workload time and memory comparison on the [locked source archive](experiments/base64-archive-workload-20260925.md) remains open. Preserve alphabets, newlines, validation and buffer formats. |
