...
| 5 | **urllib.parse** — `Lib/urllib/parse.py`; `test_urlparse.py`, `test_urllib.py` | A guarded Rust `quote_from_bytes` byte scan reaches public `quote` callers. A separate private proof accelerates exact ASCII/default-decoder `unquote`. | The installed quote candidate improved three complete catalog URL tasks beyond local noise, while Django cold/warm requests showed no established benefit. A matched-compiler unquote proof cut complete search-form wall/CPU by 43.6%/44.7% with exact digests and +368 extension bytes; it awaits reproducible opt-in source integration and public semantic qualification. Upstream memory/allocation parity remains open. See [`experiments/url-unquote-proof-20260925.md`](experiments/url-unquote-proof-20260925.md). |
