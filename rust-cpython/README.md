...
| 12 | **tarfile** — `Lib/tarfile.py`; `test_tarfile.py` | A narrow Rust byte scan could compute both header checksum conventions while `TarInfo`, PAX, file-like streaming, and extraction remain in Python. | The real locked CPython source archive has 6,540 checksum calls occupying 43.23–43.27 ms of a 581 ms instrumented complete read, a 7.4% impossible upper bound. An exact 512-byte proof is queued; full header/PAX replacement lacks a safe coarse boundary here. See [`experiments/tarfile-boundary-scout-20260925.md`](experiments/tarfile-boundary-scout-20260925.md). |
