...
| 12 | **tarfile** — `Lib/tarfile.py`; `test_tarfile.py` | An optional exact 512-byte Rust scan computes both header checksum conventions while `TarInfo`, PAX, file-like streaming, and extraction remain in Python. | The installed macOS source patch reduced five complete source-archive read pairs by a median 9.22% wall and 9.48% CPU with matching output. It remains opt-in: exact-input monkeypatch semantics, eager import cost, private-module ownership, broader behavior, and memory qualification are unresolved. The earlier instrumented 7.4% checksum share was diagnostic, not a prediction for the uninstrumented task. See [`experiments/tar-checksum-integrated-20260925.md`](experiments/tar-checksum-integrated-20260925.md). |
