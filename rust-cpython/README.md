...
tasks remained within control noise. The unstripped installed extension is
1.59 MB larger. Application breadth, memory parity, and semantic edges remain
...
| 5 | **urllib.parse** — `Lib/urllib/parse.py`; `test_urlparse.py`, `test_urllib.py` | A guarded Rust `quote_from_bytes` byte scan now reaches public `quote` callers. | The installed candidate improved three complete catalog URL tasks beyond local noise, with a small fresh-import CPU cost. Cold and warm Django WSGI requests showed no established benefit. Upstream unique/proportional memory and allocations remain unqualified. See [`experiments/django-cold-comparison-20260925.md`](experiments/django-cold-comparison-20260925.md). |
