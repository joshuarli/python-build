...
`binascii`. Also, although later research notes discuss Rust compression
backends, this exact source pin still builds `Modules/zlibmodule.c` and
`Modules/_bz2module.c`; it does **not** integrate `zlib-rs` or
`libbzip2-rs`. Those are prior art and follow-up candidates, not features of
this build. Preserve the upstream Rust-for-CPython model (`cpython-sys` plus
CPython-owned internal Rust APIs); PyO3 is useful research, not a dependency
for in-tree modules.
...
| ---: | --- | --- | --- |
| 1 | **zlib** — `Modules/zlibmodule.c`; `test_zlib.py`, `test_gzip.py`, `test_binascii.py` | Compression and decompression sit under ZIP, gzip, wheels, and HTTP. First try a backend behind the existing C/Python API. | Preserve stream states, errors, flush modes, CRC and exact byte output. `zlib-rs` and Rust-for-CPython's separate `zlib-py` are prior art; neither is wired into this pin. Compare all levels, chunk sizes, checksums, gzip/ZIP output, and whole archive workloads against system zlib. |
...
| 5 | **urllib.parse** — `Lib/urllib/parse.py`; `test_urlparse.py`, `test_urllib.py` | Batch string/byte scanning, quoting, unquoting, and query parsing in native kernels. | Control characters, malformed escapes, bytes handling, and legacy parsing differ from WHATWG behavior; Rust's `url` crate is not a drop-in. Fuzz malformed inputs and compare end-to-end URL/query workloads. |
| 6 | **json / `_json`** — `Lib/json/`, `Modules/_json.c`; `test_json/`, `test_free_threading/test_json.py` | Consider a full-document encode/decode engine with Python fallback for dynamic hooks, rather than per-token FFI calls. | The C accelerator is a strong baseline. Preserve hooks, subclasses, float formatting, escaping, recursion and error locations. `orjson`/`serde_json` are implementation references. Measure against `_json` with hooks both absent and present and representative documents. |
| 7 | **zipfile / zipimport** — `Lib/zipfile/`, `Lib/zipimport.py`; `test_zipfile/`, `test_zipfile64.py`, `test_zipimport.py` | Accelerate central-directory parsing, indexing, name decoding, CRC and bulk member processing while retaining Python file-like APIs. | ZIP64, malformed archives, codecs, import hooks, and extraction security matter. Rust ZIP crates are references. Measure import-from-ZIP, wheel inspection/install, lookup, decompression, and I/O separately; compose with zlib only after its parity is established. |
