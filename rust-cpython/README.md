...
| ---: | --- | --- | --- |
| 1 | **zlib** — `Modules/zlibmodule.c`; `test_zlib.py`, `test_gzip.py`, `test_binascii.py` | Compression and decompression sit under ZIP, gzip, wheels, and HTTP. Keep the CPython C/Python API and swap only the backend. | The whole-backend candidate changed 210 of 876 sampled compressed encodings. The separate inflate-only hybrid preserved those bytes and lowered process CPU about 36% in sustained public decode/gzip tasks. Application breadth, upstream resource parity, and its 1.45 MB stripped-size cost remain open. See [`experiments/zlib-sustained-20260925.md`](experiments/zlib-sustained-20260925.md). |
