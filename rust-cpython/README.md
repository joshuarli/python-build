...
| Rank | Area: current implementation and CPython tests | Leverage and proposed Rust boundary | Hazards, prior art, and measurement gate |
| ---: | --- | --- | --- |
| 11 | **`_strptime`** — `Lib/_strptime.py`; `test_strptime.py`, `test_datetime.py`, `test_time.py` | An optional exact `datetime.datetime.strptime` path scans fixed-width `%Y/%m/%d %H:%M:%S` ASCII input in a private Rust module, then constructs the public datetime in Python. Other inputs retain the original parser. | Five complete 60,000-record log-ingest pairs on macOS matched output and gave median candidate/control wall and kernel CPU ratios of 0.641 and 0.613; median paired peak RSS changed by +32 KiB. Cold import was neutral. Eligible calls bypass locale and regex-cache side effects, so keep `--strptime-numeric` opt-in pending semantic, memory, and Linux qualification. See [`experiments/strptime-numeric-20260925.md`](experiments/strptime-numeric-20260925.md). |
