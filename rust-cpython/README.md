...
| 5 | **urllib.parse** — `Lib/urllib/parse.py`; `test_urlparse.py`, `test_urllib.py` | A guarded `quote_from_bytes` byte scan is the first narrow hypothesis; keep public `quote` and application URL handling in view. | The registered [`catalog_url_normalize`](../benchmarks/workloads/catalog_url.py) task exercises quoting within complete URL/key normalization, but has no quiet-host baseline yet. Preserve control characters, malformed escapes, byte and Unicode handling, and legacy rules; Rust's `url` crate is not a drop-in. |
