...
| 12 | **tarfile** — `Lib/tarfile.py`; `test_tarfile.py` | An optional exact 512-byte Rust scan computes both header checksum conventions in a TAR-owned private extension while `TarInfo`, PAX, file-like streaming, and extraction remain in Python. | Five complete source-archive pairs with the lazy extension improved median wall by 9.24% and user CPU by 9.17%, with matching output; cold `tarfile` import left it unloaded. It remains opt-in: helper replacement before import can still be bypassed, and broader behavior and memory qualification are open. The earlier instrumented 7.4% checksum share was diagnostic, not a prediction for the uninstrumented task. See [`experiments/tar-owned-module-20260925.md`](experiments/tar-owned-module-20260925.md). |
