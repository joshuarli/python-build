...
| 5 | **urllib.parse** — `Lib/urllib/parse.py`; `test_urlparse.py`, `test_urllib.py` | A guarded `quote_from_bytes` byte scan is the first narrow hypothesis; keep public `quote` and application URL handling in view. | The registered [`catalog_url_normalize`](../benchmarks/workloads/catalog_url.py) task has a 0.64-second control interval and a 2.61% self-noise allowance. Most calls eligible for a native scan have inputs of at most 17 bytes, so call overhead needs a complete-workload trial. Preserve byte and Unicode handling and legacy rules; Rust's `url` crate is not a drop-in. |
