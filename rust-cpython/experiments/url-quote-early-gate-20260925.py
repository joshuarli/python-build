"""Measure whether an early short-input fallback saves URL quote guard cost."""
...
WORK = ROOT / "rust-cpython/work/url-quote-early-gate-20260925"
DATA = ROOT / "rust-cpython/experiments/data/url-quote-early-gate-20260925.json"
PATCH = ROOT / "rust-cpython/patches/0001-rust-url-quote.patch"
EXPERIMENT_PATCH = ROOT / "rust-cpython/experiments/url-quote-early-gate-20260925.patch"
...
def prepare(data: dict[str, object], evidence_path: Path) -> None:
    ...
    for side in ("pure", "current", "fallback", "early"):
    ...
    fallback = current.replace("_rust_url_type(bs) is bytes and _rust_url_type(safe) is bytes and",
                               "_rust_url_type(bs) is bytes and _rust_url_type(safe) is bytes and\n            not (safe and bs_len < 32) and")
    if fallback == current or fallback.count("not (safe and bs_len < 32)") != 1:
        raise RuntimeError("fallback guard replacement failed")
    (WORK / "fallback/urllib/parse.py").write_text(fallback)
    original = ("            _rust_url_gettrace() is None and _rust_url_getprofile() is None and\n"
                "            _rust_url_type(bs) is bytes and _rust_url_type(safe) is bytes and\n"
                "            not (safe and bs_len < 32) and")
    moved = ("            _rust_url_type(bs) is bytes and _rust_url_type(safe) is bytes and\n"
             "            not (safe and bs_len < 32) and\n"
             "            _rust_url_gettrace() is None and _rust_url_getprofile() is None and")
    early = fallback.replace(original, moved)
    if early == fallback or early.count(moved) != 1:
        raise RuntimeError("early guard replacement failed")
    (WORK / "early/urllib/parse.py").write_text(early)
    EXPERIMENT_PATCH.write_text("".join(difflib.unified_diff(
        fallback.splitlines(keepends=True), early.splitlines(keepends=True),
...
def main() -> None:
    ...
    data: dict[str, object] = {"hypothesis": "moving the exact-byte short-input fallback before trace/profile checks reduces request-path CPU",
        "rule": "after the fast exit and exact bytes type checks, nonempty safe and 1 < input length < 32 use the original cached Python quoter",
    ...
    if not args.skip_calibration:
        for length in (8, 24, 64):
            for safe in (b"", b"/", b"+", b"/:?"):
                ...
                order = ("pure", "current", "fallback", "early") if length != 24 else ("early", "fallback", "current", "pure")
    ...
    for task in EXPECTED:
        for side_a, side_b, pairs in (("fallback", "early", 2), ("current", "early", 2),
                                      ("pure", "early", 2), ("fallback", "fallback", 1),
                                      ("early", "early", 1)):
