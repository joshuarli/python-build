"""Build and measure the exact default-safe quote route."""
...
DATA = ROOT / "rust-cpython/experiments/data/url-quote-safe-slash-20260925-run2.json"
PATCH = ROOT / "rust-cpython/patches/0001-rust-url-quote.patch"
EXPERIMENT_PATCH = ROOT / "rust-cpython/experiments/url-quote-safe-slash-20260925.patch"
...
def prepare(data: dict[str, object]) -> None:
    ...
    if proposed == current or proposed.count("safe != b'/'") != 1:
    ...
    data["identities"] = {"patch": digest(PATCH), "python": digest(PYTHON),
                          ...
                          "rustc": digest(Path.home() / ".rustup/toolchains/nightly-2026-09-15-aarch64-apple-darwin/bin/rustc"),
                          "clang": digest(LLVM / "bin/clang"),
    ...
    data["build_recipe"] = {
        "rust": "nightly-2026-09-15 rustc directly --edition=2024 --crate-type=staticlib -C opt-level=3 -C panic=abort --target=aarch64-apple-darwin quote.rs -o libquote_ascii.a",
...
def long_safe_sentinel(side: str, attempt_id: str) -> dict[str, object]:
    env = dict(os.environ)
    env.update(PYTHONPATH=os.pathsep.join((str(WORK / side), str(WORK / "native"), str(ROOT))),
               PYTHONDONTWRITEBYTECODE="1", PYTHONHASHSEED="1")
    code = ("import hashlib, json, urllib.parse; "
            "value = (b'/catalog item/%' * 2200)[:32768]; "
            "outputs = [urllib.parse.quote_from_bytes(value, safe=b'/') for _ in range(100)]; "
            "print(json.dumps({'digest': hashlib.sha256(outputs[-1].encode()).hexdigest(), "
            "'output_length': len(outputs[-1]), 'input_length': len(value)}))")
    record = measured([str(PYTHON), "-B", "-c", code], env=env)
    record.update(id=attempt_id, side=side, task="long_safe_slash_synthetic", synthetic=True)
    try:
        result = json.loads(record.pop("stdout_excerpt"))
        record.update(result)
        record["valid"] = (record["returncode"] == 0 and result["input_length"] == 32768)
    except (ValueError, KeyError, TypeError):
        record["valid"] = False
    return record
