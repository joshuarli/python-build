"""Measure serial complete rational ledger and cold fractions import processes."""
...
def attempt(family: str, pair: int, position: int, side: str, stage: Path,
            env: dict[str, str]) -> dict:
    kind = "cold-import" if family == "cold-import" else "rational-ledger"
    python = stage / "bin/python3.16"
    command = ([str(python), "-S", "-B", "-c", "import fractions"]
               if kind == "cold-import" else
               [str(python), "-B", str(WORKLOAD), "--count", "100000", "--rounds", "2"])
    ...
    if result.returncode == 0:
        record["output"] = (json.loads(result.stdout) if kind == "rational-ledger" else
...
def main(control: Path, candidate: Path, output: Path, *, calibration_only: bool = False,
    ...
    for key in tuple(env):
        if key in ("PYTHONPATH", "PYTHONPYCACHEPREFIX") or key.startswith("DYLD_"):
            env.pop(key)
    cache = LANE / "work/fraction-empty-pycache"
    ...
    evidence = {"recipe": {
        "workload": "fraction_rational_workload.py --count 100000 --rounds 2",
        ...
        "control_fractions_sha256": digest(control / "lib/python3.16/fractions.py"),
        "candidate_fractions_sha256": digest(candidate / "lib/python3.16/fractions.py"),
        "candidate_extension_sha256": digest(next((candidate / "lib/python3.16/lib-dynload").glob("_rust_fraction_rational*.so"))),
        ...
        "order": ("three control/control pairs under host load" if calibration_only else
                  "one candidate/control diagnostic pair under host load" if diagnostic_pair else
                  "three self pairs and five alternating candidate pairs under host load" if loaded_host else
                  "three self pairs, five alternating comparison pairs, five alternating cold import pairs"),
    ...
    try:
        expected = None
        for family, count in ((("self", 3),) if calibration_only else
                              (("candidate", 1),) if diagnostic_pair else
                              (("self", 3), ("candidate", 5)) if loaded_host else
                              (("self", 3), ("candidate", 5), ("cold-import", 5))):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--diagnostic-pair", action="store_true")
    parser.add_argument("--loaded-host", action="store_true")
    args = parser.parse_args()
    if sum((args.calibration_only, args.diagnostic_pair, args.loaded_host)) > 1:
        parser.error("select one diagnostic mode")
    main(args.control, args.candidate, args.output,
         calibration_only=args.calibration_only,
         diagnostic_pair=args.diagnostic_pair, loaded_host=args.loaded_host)
