"""Measure serial complete command parsing and cold shlex import processes."""
...
def attempt(family: str, pair: int, position: int, side: str, stage: Path,
    ...
    kind = "cold-import" if family == "cold-import" else "command-processing"
    python = stage / "bin/python3.16"
    command = ([str(python), "-S", "-B", "-c", "import shlex"]
               if kind == "cold-import" else
               [str(python), "-B", str(WORKLOAD), "--count", "12000", "--rounds", "2"])
    ...
    if result.returncode == 0:
        record["output"] = (json.loads(result.stdout) if kind == "command-processing" else
...
def main(control: Path, candidate: Path, output: Path) -> None:
    ...
    for key in tuple(env):
        if key in ("PYTHONPATH", "PYTHONPYCACHEPREFIX") or key.startswith("DYLD_"):
            env.pop(key)
    cache = LANE / "work/shlex-empty-pycache"
    ...
    evidence = {"recipe": {
        "workload": "shlex_split_workload.py --count 12000 --rounds 2",
        ...
        "control_shlex_sha256": digest(control / "lib/python3.16/shlex.py"),
        "candidate_shlex_sha256": digest(candidate / "lib/python3.16/shlex.py"),
        "candidate_extension_sha256": digest(next((candidate / "lib/python3.16/lib-dynload").glob("_rust_shlex_split*.so"))),
    ...
    try:
        expected = None
        for family, count in (("self", 3), ("candidate", 5), ("cold-import", 5)):
