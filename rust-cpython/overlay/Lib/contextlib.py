"""Utilities for with-statement context managers."""
...
class _GeneratorContextManagerBase:
    ...
    def __init__(self, func, args, kwds):
        ...
        # Give each context manager instance the wrapped function's docstring.
    ...
    def __exit__(self, typ, value, traceback):
        if typ is None:
            ...
            try:
                ...
                # Preserve the original exception instead of raising it again.
                if exc is value:
                    exc.__traceback__ = traceback
                    return False
                # Avoid suppressing a StopIteration exception passed to throw()
                # when the generator machinery wraps it in RuntimeError. Only
                # unwrap it when the wrapped exception is Stop(Async)Iteration.
                if (
        ...
        if typ is None:
            ...
            try:
                ...
                if exc is value:
                    exc.__traceback__ = traceback
                    return False
                # Avoid suppressing a Stop(Async)Iteration exception passed to
                # athrow() when the generator machinery wraps it in RuntimeError.
                # Only unwrap it when the wrapped exception is Stop(Async)Iteration.
                if (
...
class _BaseExitStack:
    ...
    def _push_exit_callback(self, callback, is_sync=True):
        # Rust handles stack operations and callback calls; these Python
        # frames preserve with-statement tracebacks and exception chaining.
        _contextlib_rs.push_callback(self._exit_callbacks, (is_sync, callback))
...
class ExitStack(_BaseExitStack, AbstractContextManager):
    ...
    def __exit__(self, *exc_details):
        ...
        def _fix_exception_context(new_exc, old_exc):
            ...
            while 1:
                ...
                if exc_context is None or exc_context is old_exc:
                    # The context is already set correctly.
                    return
        ...
        while self._exit_callbacks:
            is_sync, cb = _contextlib_rs.pop_callback(self._exit_callbacks)
            assert is_sync
            try:
                if exc is None:
                ...
                cb = _contextlib_rs.wrap_callback(cb, True)
                if cb(*exc_details):
                    suppressed_exc = True
                    pending_raise = False
                    exc = None
        ...
        while self._exit_callbacks:
            ...
            try:
                ...
                if is_sync:
                    ...
                    cb = _contextlib_rs.wrap_callback(cb, False)
