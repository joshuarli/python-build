...
def _strptime(data_string, format="%a %b %d %H:%M:%S %Y"):
    ...
    if rust_fields is not None:
        missing = -(1 << 63)
        (iso_year, year, month, day, hour, minute, second, fraction, tz,
         gmtoff, gmtoff_fraction, iso_week, week_of_year,
         week_of_year_start, weekday, julian) = rust_fields
        iso_year = None if iso_year == missing else iso_year
        year = None if year == missing else year
        iso_week = None if iso_week == missing else iso_week
        week_of_year = None if week_of_year == missing else week_of_year
        week_of_year_start = (None if week_of_year_start == missing
                              else week_of_year_start)
        weekday = None if weekday == missing else weekday
        julian = None if julian == missing else julian
        gmtoff = None if gmtoff == missing else gmtoff
    else:
        # Locale data and the format cache remain owned by this module. Rust
        # receives only fields that the cached CPython expression already matched.
