...
_RUST_WARNINGS_UNSET = object()
_rust_warnings = _RUST_WARNINGS_UNSET


def _get_rust_warnings():
    global _rust_warnings
    if _rust_warnings is _RUST_WARNINGS_UNSET:
        try:
            import _warnings_rs
        except ImportError:
            _rust_warnings = None
        else:
            _rust_warnings = _warnings_rs
    return _rust_warnings
...
def _formatwarnmsg_impl(msg):
    ...
    rust = _get_rust_warnings()
    if rust is None:
        s = f"{msg.filename}:{msg.lineno}: {category}: {msg.message}\n"
    else:
        s = rust.format_message(msg, category)
...
def _add_filter(*item, append):
    with _wm._lock:
        ...
        if rust is None:
            ...
            rust.add_filter(filters, item, append)
        _wm._filters_mutated_lock_held()
