...
# This retained variable limits collision retries, but changing it is not a
# supported API.
template = "tmp"
...
def _mkstemp_inner(dir, pre, suf, flags, output_type):
    ...
    if _tempfile_rs is not None:
        return _tempfile_rs.mkstemp_inner(
            dir, pre, suf, flags, names, _os, _sys, TMP_MAX,
            _errno.EEXIST, next)
...
def mkdtemp(suffix=None, prefix=None, dir=None):
    ...
    if _tempfile_rs is not None:
        return _tempfile_rs.mkdtemp(
            dir, prefix, suffix, names, _os, _sys, TMP_MAX,
            _errno.EEXIST, next)
...
class TemporaryFileWrapper:
    ...
    def __getattr__(self, name):
        ...
        if hasattr(a, '__call__'):
            ...
            # Keep the closer alive while a delegated method is still reachable.
    ...
    def __iter__(self):
        # Keep the wrapper alive while iterating. Yielding from the file would
        # let generator finalization close it through the file's close method.
