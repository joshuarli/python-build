...
def _normalize(data):
    if _uuid_rs is None:
        return data
    return _uuid_rs.normalize(data)


def _set_version(data, version):
    if _uuid_rs is not None:
...
class UUID:
    ...
    def __init__(self, hex=None, bytes=None, bytes_le=None, fields=None,
        ...
        if version is not None:
            if not 1 <= version <= 8:
                raise ValueError('illegal version number')
            data = _set_version(int.to_bytes(16), version)
        else:
            data = _normalize(int.to_bytes(16))
        int = int_.from_bytes(data)
        object.__setattr__(self, 'int', int)
        object.__setattr__(self, 'is_safe', is_safe)

    @classmethod
    def _from_int(cls, value, version=None):
        """Create a UUID from an integer *value*. Internal use only."""
        assert 0 <= value <= _UINT_128_MAX, repr(value)
        data = value.to_bytes(16)
        if version is not None:
            data = _set_version(data, version)
        else:
    ...
    @property
    def bytes(self):
        return _normalize(self.int.to_bytes(16))
...
def uuid1(node=None, clock_seq=None):
    ...
    if _generate_time_safe is not None and node is clock_seq is None:
        ...
        try:
            ...
            is_safe = SafeUUID.unknown
        return UUID(bytes=uuid_time, version=1, is_safe=is_safe)
...
def uuid4():
    ...
    return UUID(bytes=_uuid_rs.uuid4())
...
def uuid6(node=None, clock_seq=None):
    ...
    return UUID._from_int(int_uuid_6, version=6)
...
def uuid7():
    ...
    res = UUID._from_int(int_uuid_7, version=7)
...
def uuid8(a=None, b=None, c=None):
    ...
    * 'c' is the last 62-bit chunk (octets 8-15).
    ...
    return UUID._from_int(int_uuid_8, version=8)
