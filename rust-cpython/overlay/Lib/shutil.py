...
# Keep stream copies in large chunks to limit read and write calls.
...
def _determine_linux_fastcopy_blocksize(infd):
    ...
    # Avoid OverflowError when a 32-bit host cannot represent larger sizes.
    if sys.maxsize < 2 ** 32:
...
def _fastcopy_copy_file_range(fsrc, fdst):
    ...
    while True:
        try:
            ...
            if n_copied == 0:
                # A zero-byte first call can mean the filesystem silently declined.
                if offset == 0:
...
def copyfileobj(fsrc, fdst, length=0):
    ...
    if not length:
        length = COPY_BUFSIZE
    if _shutil_rs is not None:
        return _shutil_rs.copyfileobj(fsrc, fdst, length)
    # Localize variable access to minimize overhead.
    fsrc_read = fsrc.read
    fdst_write = fdst.write
...
def copyfile(src, dst, *, follow_symlinks=True):
    ...
    if not follow_symlinks and _islink(src):
        ...
        with open(src, 'rb') as fsrc:
                    ...
                    # Linux / Android / Solaris
                        ...
                        if _USE_CP_SENDFILE:
                            try:
                                ...
                                pass
                    # Use readinto() for large files on Windows.
            ...
            # Report a missing destination directory with a clearer exception.
...
def copytree(src, dst, symlinks=False, ignore=None, copy_function=copy2,
    ...
    with os.scandir(src) as itr:
        entries = list(itr)
    if (_shutil_rs is not None and isinstance(src, str) and isinstance(dst, str)
            and not symlinks and ignore is None and copy_function is copy2
            and not ignore_dangling_symlinks and not dirs_exist_ok
            and all(isinstance(entry, os.DirEntry) for entry in entries)):
        errors = []

        def copy_file(entry, srcname, dstname):
            try:
                copy_function(entry, dstname)
            except Error as err:
                errors.extend(err.args[0])
            except OSError as why:
                errors.append((srcname, dstname, str(why)))

        def copy_stat(srcname, dstname):
            try:
        ...
        copied = _shutil_rs.copytree(
            src, dst, copy_file, copy_stat, entries, _copytree_makedirs)
...
def _copytree_makedirs(path):
    return os.makedirs(path, exist_ok=False)
...
# This path-based implementation cannot protect against symlink races.
def _rmtree_unsafe(path, dir_fd, onexc):
    ...
    try:
        if _rmtree_islink(st):
            # Following a directory symlink could remove a different tree.
    ...
    try:
        ...
        try:
            if not os.path.samestat(orig_st, os.fstat(topfd)):
                # Reject a symlink discovered between the lstat() and open().
                raise OSError("Cannot call rmtree on a symbolic link")
        ...
        if path is None:
            ...
            if path is None:
                try:
                    ...
                    path = os.defpath
            # An explicitly empty PATH means there are no search locations.
