...
Python tokens.  It decodes the bytes according to source encoding cookies
when determining source file encoding.
...
def detect_encoding(readline):
    ...
    cookie.  If both a bom and a cookie are present,
...
_RUST_TOKEN_TYPES = (NAME, NUMBER, OP, NEWLINE)
_RUST_SCAN_MAX_LINES = 128
_RUST_SCAN_MAX_CHARS = 32768


def _c_tokenizer_tokens(source, encoding, extra_tokens, buffered, skip):
    ...
    if encoding is None:
        it = _tokenize.TokenizerIter(replay_source, extra_tokens=extra_tokens)
    else:
        it = _tokenize.TokenizerIter(
            replay_source, encoding=encoding, extra_tokens=extra_tokens
        )
    try:
        for index, info in enumerate(it):
            if index >= skip:
...
def _generate_tokens_from_c_tokenizer(source, encoding=None, extra_tokens=False):
    """Tokenize simple source lines in Rust, falling back to the C tokenizer."""
    if not extra_tokens:
        yield from _c_tokenizer_tokens(source, encoding, extra_tokens, (), 0)
        return
    ...
    while True:
        ...
        is_eof = (type(line) is str and not line) or (
            type(line) is bytes and not line
        )
        if is_eof:
            yield TokenInfo(ENDMARKER, '', (row + 1, 0), (row + 1, 0), '')
            return

        if encoding is None and type(line) is str:
            rust_line = line
        elif encoding is not None and type(line) is bytes and line.isascii():
            rust_line = line.decode('ascii')
        else:
            yield from _c_tokenizer_tokens(
                source, encoding, extra_tokens, buffered, emitted
            )
            return

        buffered_chars += len(rust_line)
        if (len(buffered) > _RUST_SCAN_MAX_LINES
                or buffered_chars > _RUST_SCAN_MAX_CHARS):
            yield from _c_tokenizer_tokens(
                source, encoding, extra_tokens, buffered, emitted
            )
            return

        tokens = _tokenize_rs.scan_line(rust_line)
        if tokens is None:
            yield from _c_tokenizer_tokens(
                source, encoding, extra_tokens, buffered, emitted
            )
        ...
        for kind, start, end in tokens:
            token_type = _RUST_TOKEN_TYPES[kind]
            yield TokenInfo(
                token_type,
                rust_line[start:end],
                (row + 1, start),
