...
The APIs cover finding installed distributions, reading their metadata, and
resolving entry points declared by packages.
"""
...
class EntryPoint:
    """A named import target declared by an installed distribution.
...
class Distribution(metaclass=abc.ABCMeta):
    ...
    def read_text(self, filename) -> str | None:
        ...
        - entry_points.txt: Named import targets grouped by entry point kind.
        - RECORD: A comma-separated list of installed files and optional hashes.
    ...
    def metadata(self) -> _meta.PackageMetadata:
        ...
        metadata per the
        Core metadata fields describe the distribution name, version,
        dependencies, and other package attributes.
    ...
    def _assemble_message(text: str) -> _meta.PackageMetadata:
        ...
        if _metadata_rs is not None:
            parsed = _metadata_rs.parse_metadata(text)
            if parsed is not None:
                headers, payload = parsed
                message = email.message.Message()
                message._headers = headers
                message.set_payload(payload)
                return _adapters.Message(message)
    ...
    def _convert_egg_info_reqs_to_simple_reqs(sections):
        ...
        def url_req_space(req):
            """
            A direct URL requirement needs a separating space before its
            environment marker.
            """
    ...
    def _load_json(self, filename):
        # Load JSON only when a caller requests this metadata file.
        import json
...
def _clear_after_fork(cached):
    ...
    and trigger ``BadZipFile``/``OSError`` failures. Registering
    ``cache_clear`` with ``os.register_at_fork`` keeps each process on its own
    cache.
    """
...
class FastPath:
    ...
    def zip_children(self):
        # Import zipfile only after the filesystem lookup fails.
        import zipfile
...
class Prepared:
    ...
    def normalize(name):
        """
        Lowercase names, replace dashes and dots with underscores, and
        condense repeated underscores.
        """
...
class MetadataPathFinder(DistributionFinder):
    ...
    def _search_paths(cls, name, paths):
        ...
        for path in paths:
            rust_path = (
                _metadata_rs is not None
                and type(os.listdir) is types.BuiltinFunctionType
                and os.listdir is _os_listdir
            ...
            if rust_path:
                children = _metadata_rs.scan_path(path, name or '', os.path.basename(path))
                if children is not None:
                    yield from (pathlib.Path(path, child) for child in children)
                    continue
            yield from FastPath(path).search(prepared)
...
def _get_toplevel_name(name: PackagePath) -> str:
    ...
    'foo.dist-info'
    """
    # Import inspect only when file names do not provide a module name.
    import inspect
