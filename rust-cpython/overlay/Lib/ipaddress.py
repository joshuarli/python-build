...
try:
    import _ipaddress_rs as _ipaddress_rs
except ImportError:
    _ipaddress_rs = None
...
class _BaseNetwork(_IPAddressBase):
    ...
    def broadcast_address(self):
        packed = self.network_address.packed
        width = len(packed)
        if _ipaddress_rs is not None:
            address_range = _ipaddress_rs.network_bounds(packed, self._prefixlen)
            broadcast = int.from_bytes(address_range[width:], 'big')
        else:
            broadcast = int(self.network_address) | int(self.hostmask)
        return self._address_class(broadcast)
...
class _BaseV4:
    ...
    def _ip_int_from_string(cls, ip_str):
        ...
        if isinstance(ip_str, str):
            if _ipaddress_rs is not None:
                try:
                    return int.from_bytes(_ipaddress_rs.parse_ipv4(ip_str), 'big')
                except ValueError:
                    pass
    ...
    def __init__(self, address, strict=True):
        ...
        if _ipaddress_rs is not None:
            network = int.from_bytes(
                _ipaddress_rs.network_bounds(self.network_address.packed,
                                             self._prefixlen)[:4], 'big')
        else:
            network = packed & int(self.netmask)
        if network != packed:
            if strict:
                raise ValueError('%s has host bits set' % self)
            else:
                self.network_address = IPv4Address(network)
...
class _BaseV6:
    ...
    def _ip_int_from_string(cls, ip_str):
        ...
        if isinstance(ip_str, str):
            if _ipaddress_rs is not None:
                try:
                    return int.from_bytes(_ipaddress_rs.parse_ipv6(ip_str), 'big')
                except ValueError:
                    pass
    ...
    def __init__(self, address, strict=True):
        ...
        if network != packed:
            if strict:
                ...
                self.network_address = IPv6Address(network)
