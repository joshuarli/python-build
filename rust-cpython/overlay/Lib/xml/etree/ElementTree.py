...
_RUST_XML_STREAM_TYPES = (
    io.BufferedReader, io.BufferedRandom, io.FileIO,
    io.TextIOWrapper, io.BytesIO, io.StringIO,
)

try:
    import _elementtree_rs
except ImportError:
    _elementtree_rs = None

_RUST_PARSE_UNAVAILABLE = object()


def _rust_parse(data):
    ...
    *file* is an optional file handle or file name of an XML file whose
    ...
    def parse(self, source, parser=None):
        ...
        *source* is a file name or file object, *parser* is an optional
        ...
        try:
            if parser is None:
                if (_elementtree_rs is not None
                        and _RUST_HAS_C_ACCELERATOR
                        and XMLParser is _RUST_DEFAULT_XMLPARSER
                        and TreeBuilder is _RUST_DEFAULT_TREEBUILDER
                        and TreeBuilder.comment is _RUST_DEFAULT_TREEBUILDER_COMMENT
                        and TreeBuilder.pi is _RUST_DEFAULT_TREEBUILDER_PI
                        and type(source) in _RUST_XML_STREAM_TYPES):
                    chunks = []
                    while data := source.read(65536):
                        chunks.append(data)
                    if chunks and all(type(chunk) is bytes for chunk in chunks):
                        data = b"".join(chunks)
                    elif chunks and all(type(chunk) is str for chunk in chunks):
                        data = "".join(chunks)
                    else:
                        data = None
                    root = _rust_parse(data)
                    if root is not _RUST_PARSE_UNAVAILABLE:
                        self._root = root
                        return self._root
                    parser = XMLParser()
                    for chunk in chunks:
...
try:
    ...
    _RUST_HAS_C_ACCELERATOR = False
else:
    _RUST_HAS_C_ACCELERATOR = True
    _set_factories(Comment, ProcessingInstruction)

_RUST_DEFAULT_ESCAPE_CDATA = _escape_cdata
_RUST_DEFAULT_ESCAPE_ATTRIB = _escape_attrib
_RUST_DEFAULT_XMLPARSER = XMLParser
_RUST_DEFAULT_TREEBUILDER = TreeBuilder
_RUST_DEFAULT_TREEBUILDER_COMMENT = TreeBuilder.comment
_RUST_DEFAULT_TREEBUILDER_PI = TreeBuilder.pi
