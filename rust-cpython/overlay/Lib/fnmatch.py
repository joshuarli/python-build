...
def _compile_pattern(pat):
    rust = _rust_implementation()
    if rust is not None:
        rust.prepare(pat)
        return lambda name: rust.match(name, pat)

    if isinstance(pat, bytes):
        pat_str = str(pat, 'ISO-8859-1')
        res_str = _translate_regex(pat_str)
    ...
    def match(name):
        ...
        if rust is not None:
            rust.prepare(pat)
            return rust.match(name, pat)
        return fallback(name) is not None
...
def translate(pat):
    ...
    rust = _rust_implementation()
    if rust is not None:
        return rust.translate(pat)
    return _translate_regex(pat)
