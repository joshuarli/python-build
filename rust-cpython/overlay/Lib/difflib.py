...
def _rust_sequence_values(sequence):
    """Return built-in sequences whose equality and hashing have no callbacks."""
    if type(sequence) not in (str, bytes, list, tuple):
        return None
    values = tuple(sequence)
    if all(type(value) in (str, int) for value in values):
        return values
    return None
...
class SequenceMatcher:
    ...
    def find_longest_match(self, alo=0, ahi=None, blo=0, bhi=None):
        ...
        if bhi is None:
            bhi = len(b)
        if _difflib_rs is not None and self.isjunk is None:
            rust_match = self._rust_find_longest_match(alo, ahi, blo, bhi)
            if rust_match is not None:
                return Match._make(rust_match)
    ...
    def _rust_find_longest_match(self, alo, ahi, blo, bhi):
        if any(type(value) is not int for value in (alo, ahi, blo, bhi)):
            return None
        if not (0 <= alo <= ahi <= len(self.a)
                and 0 <= blo <= bhi <= len(self.b)):
            return None
...
class Differ:
    ...
    def _fancy_replace(self, a, alo, ahi, b, blo, bhi):
        ...
        # could grow very deep, and runtime cubic.
        #
...
def _format_range_unified(start, stop):
    ...
    # Unified diff ranges use one-based starts except for empty ranges.
...
def _format_range_context(start, stop):
    'Convert range to the "ed" format'
    # Context diff ranges use one-based inclusive endpoints.
    beginning = start + 1     # lines start numbering with one
    length = stop - start
    if not length:
        beginning -= 1        # empty ranges begin at line just before the range
