...
def _is_unicode_scalar_string(value):
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def _parse_optional(self, arg_string):
        if (
            _argparse_rs is not None
            and type(arg_string) is str
            and arg_string.startswith('-')
            and type(self.prefix_chars) is str
            and self.prefix_chars == '-'
            and type(self.allow_abbrev) is bool
            and type(self._option_string_actions) is dict
            and type(self).__getattribute__ is object.__getattribute__
            and not any(
                name in type(self).__dict__
                for name in ('prefix_chars', 'allow_abbrev', '_option_string_actions')
            )
            and type(self)._get_option_tuples is ArgumentParser._get_option_tuples
            and '_get_option_tuples' not in vars(self)
            and _is_unicode_scalar_string(arg_string)
            and all(
                type(option) is str and _is_unicode_scalar_string(option)
                for option in self._option_string_actions
            )
        ):
            candidates = _argparse_rs.option_candidates(
                arg_string,
                tuple(self._option_string_actions),
                self.allow_abbrev,
            )
            if candidates:
                option_tuples = []
                for option, sep, explicit_arg in candidates:
                    if sep == '=':
                        option_head, sep, explicit_arg = arg_string.partition('=')
                        if option == option_head:
                            option = option_head
                        else:
                            option = next(
                                key for key in self._option_string_actions
                                if key == option
                            )
                    elif sep == '':
                        option = next(
                        ...
                        explicit_arg = arg_string[2:]
                    elif option == arg_string:
                        option = arg_string
                    else:
                        option = next(
                            key for key in self._option_string_actions
                    ...
                    option_tuples.append(
                        (self._option_string_actions[option], option, sep, explicit_arg)
                    )
