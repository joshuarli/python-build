...
try:
    import _html_parser_rs
except ImportError:
    _html_parser_rs = None
...
# Keep the tolerant tag and attribute expressions in sync with locatetagend;
# the parser uses all three expressions to recognize the same start-tag form.
...
class HTMLParser(_markupbase.ParserBase):
    ...
    def goahead(self, end):
        if _html_parser_rs is None:
            return self._goahead_python(end)

        rawdata = self.rawdata
        i = 0
        n = len(rawdata)
        # A callback can change CDATA handling for the next token.
        while i < n:
            kind, token_end = _html_parser_rs.next_token(
                rawdata, i, end, self.convert_charrefs, self.cdata_elem,
                self._escapable, self._support_cdata)
            if kind == 'incomplete':
                if not end:
                    break
                if starttagopen.match(rawdata, i):
                    ...
                    for suffix in ("--!", "--", "-"):
                        if rawdata.endswith(suffix, i+4):
                            ...
                            break
                    self.handle_comment(rawdata[i+4:j])
                elif rawdata.startswith("<![CDATA[", i) and self._support_cdata:
                    self.unknown_decl(rawdata[i+3:])
                elif rawdata[i:i+9].lower() == '<!doctype':
                    self.handle_decl(rawdata[i+2:])
                elif rawdata.startswith("<!", i):
                    ...
                    raise AssertionError("Rust scanner returned an unexpected incomplete token")
                token_end = n
            elif kind == 'data':
                if self.convert_charrefs and self._escapable:
                    self.handle_data(unescape(rawdata[i:token_end]))
                else:
                    self.handle_data(rawdata[i:token_end])
            elif kind == 'starttag':
                token_end = self.parse_starttag(i)
            elif kind == 'endtag':
                token_end = self.parse_endtag(i)
            elif kind == 'comment':
                token_end = self.parse_comment(i)
            elif kind == 'pi':
                token_end = self.parse_pi(i)
            elif kind == 'declaration':
                token_end = self.parse_html_declaration(i)
            elif kind == 'charref':
                name_end = token_end
                if rawdata.startswith(';', token_end - 1):
                    name_end -= 1
                self.handle_charref(rawdata[i+2:name_end])
            elif kind == 'incomplete_charref':
                self.handle_charref(rawdata[i+2:token_end])
            elif kind == 'entityref':
                name_end = token_end
                if rawdata.startswith(';', token_end - 1):
                    name_end -= 1
                self.handle_entityref(rawdata[i+1:name_end])
            elif kind != 'ignored':
                raise AssertionError("Rust scanner returned an unknown token")

            if token_end < 0:
                self.rawdata = rawdata[i:]
                return self._goahead_python(end)
            i = self.updatepos(i, token_end)
    ...
    # Handle declarations using HTML's doctype and CDATA rules.
    ...
    def parse_endtag(self, i):
        ...
        # Ignore malformed trailing attributes after extracting the tag name.
