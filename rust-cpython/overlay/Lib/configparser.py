...
def _rust_backend():
    try:
        ...
        # Keep the Python implementation available in incompatible contexts.
        return None
    return _configparser_rs


def _uses_standard_rust_backend(parser):
    if type(parser) not in (RawConfigParser, ConfigParser):
        return False
    if parser._dict is not _default_dict:
        return False
    ...
    if parser._allow_no_value or not parser._strict:
        return False
    if not parser._empty_lines_in_values or parser._allow_unnamed_section:
        return False
    if getattr(parser.optionxform, '__func__', None) is not RawConfigParser.optionxform:
        return False
    if type(parser._interpolation) not in (Interpolation, BasicInterpolation):
        return False
    section = parser.default_section
    if (type(section) is not str or not section or section != section.strip()
            or '[' in section or ']' in section or '\n' in section or '\r' in section):
        return False
    return True
...
RawConfigParser.write = _write_with_rust_backend
