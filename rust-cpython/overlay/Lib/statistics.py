...
def _get_rust_statistics():
    global _rust_statistics
    if _rust_statistics is _RUST_STATISTICS_NOT_LOADED:
        try:
            import _statistics_rs
        except ImportError:
            _rust_statistics = None
        else:
            _rust_statistics = _statistics_rs
    return _rust_statistics


def _integer_moments(data):
    rust_statistics = _get_rust_statistics()
    if rust_statistics is None:
        return None
    result = rust_statistics.integer_moments(data)
    if result is False:
        return None
    return result


def _median_indices(data):
    rust_statistics = _get_rust_statistics()
    if rust_statistics is None:
        return None
    result = rust_statistics.median_indices(data)
    if result is False:
        return None
    return result
