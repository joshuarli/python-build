...
# If available, use the native implementation for low-level heap operations.
try:
    from _heapq import *
except ImportError:
    _heapq_module = 'heapq'
else:
    _heapq_module = '_heapq'

# Keep comparisons, object ownership, and list mutation at the CPython
# boundary while the heap algorithms run in Rust.
try:
    import _heapq_rs as _rust_heapq
except ImportError:
    _rust_heapq = None

if _rust_heapq is not None:
    _rust_heapq_docs = {
        name: globals()[name].__doc__
        for name in ('heappush', 'heappop', 'heapify', 'heapreplace',
                     'heappushpop', 'heappush_max', 'heappop_max',
                     'heapify_max', 'heapreplace_max', 'heappushpop_max')
    }

    def heappush(heap, item, /):
        _rust_heapq.heappush(heap, item)

    def heappop(heap, /):
        return _rust_heapq.heappop(heap)

    def heapify(heap, /):
        _rust_heapq.heapify(heap)

    def heapreplace(heap, item, /):
        return _rust_heapq.heapreplace(heap, item)

    def heappushpop(heap, item, /):
        return _rust_heapq.heappushpop(heap, item)

    def heappush_max(heap, item, /):
        _rust_heapq.heappush_max(heap, item)

    def heappop_max(heap, /):
        return _rust_heapq.heappop_max(heap)

    def heapify_max(heap, /):
        _rust_heapq.heapify_max(heap)

    def heapreplace_max(heap, item, /):
        return _rust_heapq.heapreplace_max(heap, item)

    def heappushpop_max(heap, item, /):
        return _rust_heapq.heappushpop_max(heap, item)

    for _name in ('heappush', 'heappop', 'heapify', 'heapreplace',
                  'heappushpop', 'heappush_max', 'heappop_max',
                  'heapify_max', 'heapreplace_max', 'heappushpop_max'):
        _function = globals()[_name]
        _function.__module__ = _heapq_module
        _function.__doc__ = _rust_heapq_docs[_name]
    del _function, _name, _rust_heapq_docs
del _heapq_module
