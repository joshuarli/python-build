...
try:
    import _logging_rs as _logging_native
except ImportError:
    _logging_native = None
...
def getLevelName(level):
    ...
    """
    # Levels may be registered by name or numeric value, so check both maps.
...
# have __file__ set, so we get the
...
def _prepareFork():
    ...
    # abandoned in the event of an asynchronous exception.
    try:
        _lock.acquire()
...
class LogRecord(object):
    ...
    def __init__(self, name, level, pathname, lineno,
        ...
        # problem.
        # String mapping substitutions require a mapping object, so accept
        # Mapping rather than arbitrary objects with a __getitem__ method.
        ...
        # Keep milliseconds as a float for compatibility with existing records.
        ...
        if not logMultiprocessing: # pragma: no cover
            ...
            if mp is not None:
                # Errors may occur while multiprocessing is initializing, for
                # example when an import hook runs application code during import.
                try:
    ...
    def getMessage(self):
        ...
        """
        if _logging_native is not None:
            return _logging_native.get_message(self.msg, self.args)
        msg = str(self.msg)
        if self.args:
            msg = msg % self.args
        return msg
    ...
    def handleError(self, record):
        ...
        if raiseExceptions and sys.stderr:
            ...
            try:
                ...
                # Include the original message and arguments when formatting fails.
                try:
                ...
                # Preserve the original failure if writing this report fails.
                pass
            finally:
                del exc
...
class StreamHandler(Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            stream = self.stream
            # Write one complete record so custom streams see a single call.
...
class FileHandler(StreamHandler):
    ...
    def __init__(self, filename, mode='a', encoding=None, delay=False, errors=None):
        ...
        """
        # Accept path-like objects while keeping a filesystem string internally.
    ...
    def close(self):
        ...
        with self.lock:
            try:
                ...
                # Always close the handler, including when its stream was
                # delayed, so the base class updates its closed state.
    ...
    def emit(self, record):
        ...
        will not be emitted after the handler has closed.
        """
