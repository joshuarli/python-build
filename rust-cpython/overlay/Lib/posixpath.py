...
def _load_posixpath_rs():
    ...
    if not _posixpath_rs_attempted:
        ...
        if (getattr(getattr(module, '__spec__', None), '_initializing', False)
            ...
            return None
        _posixpath_rs_attempted = True
        try:
            import _posixpath_rs as extension
        except ImportError:
            pass
        else:
            _posixpath_rs = extension
    return _posixpath_rs
...
def join(a, /, *p):
    ...
    try:
        for b in p:
            b = os.fspath(b)
            if (type(a) in (str, bytes)
                    and type(path) is type(a)
                    and type(b) is type(a)):
                rust = _load_posixpath_rs()
                if rust is not None:
                    path = rust.join_pair(path, b)
                    continue
...
try:
    from posix import _path_splitroot_ex as _path_splitroot_ex
except ImportError:
    _path_splitroot_ex = None

if _path_splitroot_ex is not None:
    ...
    def splitroot(p, /):
        ...
        if p[:1] != sep:
            ...
            # Preserve the implementation-defined root made by exactly two slashes.
    ...
    if i == 1:
        if 'HOME' not in os.environ:
            ...
            try:
                ...
                # Return the path unchanged if the current user is missing from
        ...
        try:
            ...
            # Return the path unchanged if the requested user is missing from
            # the password database.
            return path
...
try:
    from posix import _path_normpath as _path_normpath
except ImportError:
    _path_normpath = None

if _path_normpath is not None:
