...
def _rust_backend():
    try:
        import _fractions_rs
    except ImportError:
        return None
    return _fractions_rs
...
class Fraction(numbers.Rational):
    ...
    def __new__(cls, numerator=0, denominator=None):
        ...
        if denominator is None:
            if type(numerator) is int:
                ...
                if backend is None:
                    ...
                    self._numerator, self._denominator = backend.parse_parts(
                        -1 if m.group('sign') == '-' else 1,
                        int(m.group('num') or '0'),
                        int(denom) if denom else 0,
                        int(decimal.replace('_', '')) if decimal else 0,
                        len(decimal.replace('_', '')),
                        int(m.group('exp')) if m.group('exp') else 0,
                        bool(denom),
                        bool(decimal),
                    )
                    return self
        ...
        backend = _rust_backend()
        if backend is not None and type(numerator) is int and type(denominator) is int:
            self._numerator, self._denominator = backend.normalize(numerator, denominator)
        else:
    ...
    def _add(a, b):
        ...
        if backend is not None:
            return Fraction._from_coprime_ints(*backend.add(
    ...
    def _sub(a, b):
        ...
        backend = _rust_backend()
        if backend is not None:
            return Fraction._from_coprime_ints(*backend.sub(
                a._numerator, a._denominator, b._numerator, b._denominator))
        na, da = a._numerator, a._denominator
        nb, db = b._numerator, b._denominator
        g = math.gcd(da, db)
    ...
    def _mul(a, b):
        ...
        backend = _rust_backend()
        if backend is not None:
            return Fraction._from_coprime_ints(*backend.mul(
                a._numerator, a._denominator, b._numerator, b._denominator))
        na, da = a._numerator, a._denominator
        nb, db = b._numerator, b._denominator
        g1 = math.gcd(na, db)
    ...
    def _div(a, b):
        ...
        backend = _rust_backend()
        if backend is not None:
            return Fraction._from_coprime_ints(*backend.truediv(
                a._numerator, a._denominator, nb, db))
        na, da = a._numerator, a._denominator
        g1 = math.gcd(na, nb)
        if g1 > 1:
            na //= g1
    ...
    def _divmod(a, b):
        ...
        backend = _rust_backend()
        if backend is not None:
            div, numerator, denominator = backend.divmod(
                a._numerator, a._denominator, b._numerator, b._denominator)
            return div, Fraction._from_coprime_ints(numerator, denominator)
    ...
    def _mod(a, b):
        ...
        backend = _rust_backend()
        if backend is not None:
            return Fraction._from_coprime_ints(*backend.mod(
                a._numerator, a._denominator, b._numerator, b._denominator))
        da, db = a.denominator, b.denominator
