...
def _unquote_impl(string: bytes | bytearray | str) -> bytes | bytearray:
    ...
    if _rust_parse is not None and isinstance(string, bytes):
        return _rust_parse.unquote_to_bytes(string)
    bits = string.split(b'%')
    if len(bits) == 1:
        return string
...
def parse_qsl(qs, keep_blank_values=False, strict_parsing=False,
    ...
    if _rust_parse is not None and type(qs) is str:
        if strict_parsing:
            for name_value in qs.split(separator):
                if name_value or strict_parsing:
                    _, has_eq, _ = name_value.partition(eq)
                    if not has_eq:
                        raise ValueError("bad query field: %r" % (name_value,))
        try:
            query_bytes = qs.encode('utf-8')
            separator_bytes = separator.encode('utf-8')
        except UnicodeEncodeError:
            query_bytes = None
            separator_bytes = None
        if (separator == '&' and type(encoding) is str
                and encoding == 'utf-8' and type(errors) is str
                and errors == 'replace'
                and query_bytes is not None):
            pairs = _rust_parse.parse_qsl_utf8(query_bytes)
            return [(name.decode('utf-8'), value.decode('utf-8'))
                    for name, value in pairs if value or keep_blank_values]

    if (_rust_parse is not None and type(qs) in (str, bytes)
            and (type(qs) is bytes or query_bytes is not None)):
        if strict_parsing and type(qs) is bytes:
            for name_value in qs.split(separator):
                if name_value or strict_parsing:
                    _, has_eq, _ = name_value.partition(eq)
                    if not has_eq:
                        raise ValueError("bad query field: %r" % (name_value,))
        if type(qs) is str:
            pairs = _rust_parse.parse_qsl(query_bytes, separator_bytes)
            pairs = [(name.decode('utf-8'), value.decode('utf-8'))
                     for name, value in pairs]
            unquote = lambda value: unquote_plus(value, encoding=encoding, errors=errors)
        else:
            pairs = _rust_parse.parse_qsl(qs, separator)
            unquote = lambda value: unquote_to_bytes(value.replace(b'+', b' '))
        r = []
        for name, value in pairs:
            if value or keep_blank_values:
                r.append((unquote(name), unquote(value)))
        return r
...
def quote_from_bytes(bs, safe='/'):
    ...
    if _rust_parse is not None:
        if type(bs) is bytearray:
            bs = bytes(bs)
        if type(bs) is bytes:
            return _rust_parse.quote_from_bytes(bs, safe).decode('ascii')
