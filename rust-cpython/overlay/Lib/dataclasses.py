...
try:
    from _dataclasses_rs import merge_fields as _merge_fields
    from _dataclasses_rs import set_field as _set_field
except ImportError:
    _merge_fields = None
    _set_field = None
...
class Field:
    ...
    # Preserve descriptor initialization before _process_class replaces
    # this Field object with the descriptor default.
    #
...
def _process_class(cls, init, repr, eq, order, unsafe_hash, frozen,
    ...
    for b in cls.__mro__[-1:0:-1]:
        ...
        if base_fields is not None:
            ...
            if _merge_fields is None:
                for f in base_fields.values():
                    fields[f.name] = f
            else:
                _merge_fields(fields, base_fields.values())
...
def _asdict_inner(obj, dict_factory):
    ...
    # handle the builtin types first for speed; subclasses handled below
        ...
        if hasattr(obj, '_fields'):
            ...
            # called with named field values rather than as an iterable.
...
def _astuple_inner(obj, tuple_factory):
    if type(obj) in _ATOMIC_TYPES:
        ...
        # treated (see below), but we just need to create them
        # differently because a namedtuple's __init__ needs to be
        # called with positional field values rather than an iterable.
