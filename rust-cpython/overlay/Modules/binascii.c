...
binascii_exec(PyObject *module)
{
    ...
    /* Keep the module's public error classes while dispatching core binary
       conversions and checksums to the per-interpreter Rust extension. */
    PyObject *rust_module = PyImport_ImportModule("_binascii_rs");
    if (rust_module == NULL) {
        return -1;
    }
    PyObject *module_name = PyObject_GetAttrString(module, "__name__");
    if (module_name == NULL) {
        Py_DECREF(rust_module);
        return -1;
    }
    static const char *const rust_methods[] = {
        "hexlify", "b2a_hex", "unhexlify", "a2b_hex", "crc32", "crc_hqx",
        NULL,
    };
    for (const char *const *name = rust_methods; *name != NULL; name++) {
        PyObject *method = PyObject_GetAttrString(rust_module, *name);
        if (method == NULL ||
            PyObject_SetAttrString(method, "__module__", module_name) < 0 ||
            PyObject_SetAttrString(module, *name, method) < 0) {
            Py_XDECREF(method);
