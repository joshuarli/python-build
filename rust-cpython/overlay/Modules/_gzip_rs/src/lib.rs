...
const COMPRESSOR_CAPSULE: &std::ffi::CStr = c"_gzip_rs.Compressor";
const DECOMPRESSOR_CAPSULE: &std::ffi::CStr = c"_gzip_rs.Decompressor";
...
pub extern "C" fn _gzip_rs_clear(_object: *mut PyObject) -> c_int {
    0
}

pub extern "C" fn _gzip_rs_free(_object: *mut c_void) {}
...
pub static _GZIP_RS_MODULE: ModuleDef = ModuleDef {
    ffi: UnsafeCell::new(PyModuleDef {
        m_base: PyModuleDef_HEAD_INIT,
        m_name: c"_gzip_rs".as_ptr() as *mut _,
        m_doc: c"Rust raw-DEFLATE codecs for gzip.".as_ptr() as *mut _,
        m_size: 0,
        m_methods: &_GZIP_RS_MODULE_METHODS as *const PyMethodDef as *mut _,
        m_slots: ptr::null_mut(),
        m_traverse: None,
        m_clear: Some(_gzip_rs_clear),
        m_free: Some(_gzip_rs_free),
    }),
};

#[unsafe(no_mangle)]
pub unsafe extern "C" fn PyInit__gzip_rs() -> *mut PyObject {
    _GZIP_RS_MODULE.init()
}
