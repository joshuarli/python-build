...
typedef struct {
    ...
    /* Optional Rust stream state for the default, dictionary-free path */
    PyObject *rust_module;
    PyObject *rust_encoder;
    ...
    if (options == Py_None && zstd_dict == Py_None) {
        ...
        if (module != NULL) {
            PyObject *encoder = PyObject_CallMethod(
                module, "encoder_new", "i", self->compression_level);
            if (encoder != NULL) {
                self->rust_module = module;
                self->rust_encoder = encoder;
            }
            else {
                Py_DECREF(module);
                PyErr_Clear();
...
ZstdCompressor_traverse(PyObject *ob, visitproc visit, void *arg)
{
    ZstdCompressor *self = ZstdCompressor_CAST(ob);
    Py_VISIT(self->dict);
    Py_VISIT(self->rust_module);
    Py_VISIT(self->rust_encoder);
    return 0;
}
