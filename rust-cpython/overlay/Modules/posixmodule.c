...
/* Keep the public path methods as posix built-ins.  The Python loader delays
 * the extension import while site initializes and returns None where the
 * extension cannot load, including in an isolated interpreter. */
static PyObject *
posixpath_rust_lexical(const char *method_name, PyObject *path)
{
    PyObject *modules = PyImport_GetModuleDict();
    PyObject *posixpath = PyDict_GetItemString(modules, "posixpath");
    if (posixpath == NULL) {
        return NULL;
    }

    PyObject *loader = PyObject_GetAttrString(posixpath, "_load_posixpath_rs");
    if (loader == NULL) {
    ...
    PyObject *extension = PyObject_CallNoArgs(loader);
    Py_DECREF(loader);
    if (extension == NULL) {
        return NULL;
    }
    ...
    PyObject *method = PyObject_GetAttrString(extension, method_name);
    Py_DECREF(extension);
    if (method == NULL) {
        return NULL;
    }
    PyObject *result = PyObject_CallOneArg(method, path);
    Py_DECREF(method);
    return result;
}
...
{
    PyObject *rust_result = posixpath_rust_lexical("_path_splitroot_ex", path->object);
    if (rust_result != NULL) {
        return rust_result;
    }
    if (PyErr_Occurred()) {
...
{
    PyObject *rust_result = posixpath_rust_lexical("_path_normpath", path->object);
    if (rust_result != NULL) {
        return rust_result;
    }
    if (PyErr_Occurred()) {
