...
const COMPRESSOR_CAPSULE: &std::ffi::CStr = c"_zip_rs.Compressor";
const DECOMPRESSOR_CAPSULE: &std::ffi::CStr = c"_zip_rs.Decompressor";
...
fn decompress_bytes(
    ...
    if !decompressor.pending_input.is_empty()
        && input.starts_with(&decompressor.pending_input)
    {
        decompressor.pending_input.clear();
    }
...
unsafe extern "C" fn crc32(
    ...
    hasher.update(&input);
    unsafe { PyLong_FromUnsignedLong(hasher.finalize() as std::ffi::c_ulong) }
}
...
unsafe extern "C" fn decompressor_decompress(
    ...
    match decompress_bytes(unsafe { &mut *decompressor }, &input, max_length as usize) {
        Ok(output) => unsafe { bytes_from_slice(&output) },
        Err(()) => {
            set_value_error(c"invalid raw DEFLATE stream");
            ptr::null_mut()
        }
    }
}
...
unsafe extern "C" fn decompressor_unconsumed_tail(
    ...
    let unconsumed_tail = unsafe { (*decompressor).pending_input.clone() };
    unsafe { bytes_from_slice(&unconsumed_tail) }
}
...
pub static _ZIP_RS_MODULE_METHODS: [PyMethodDef; 11] = [
    PyMethodDef {
        ml_name: c"crc32".as_ptr() as *mut c_char,
        ml_meth: PyMethodDefFuncPointer { PyCFunctionFast: crc32 },
        ml_flags: METH_FASTCALL,
        ml_doc: c"Compute a seeded ZIP CRC-32 checksum.".as_ptr() as *mut c_char,
    },
    PyMethodDef {
        ml_name: c"compressor".as_ptr() as *mut c_char,
        ml_meth: PyMethodDefFuncPointer { PyCFunctionFast: compressor_new },
        ml_flags: METH_FASTCALL,
        ml_doc: c"Create a raw-DEFLATE compressor.".as_ptr() as *mut c_char,
    },
    PyMethodDef {
    ...
    PyMethodDef {
        ...
        ml_flags: METH_FASTCALL,
        ml_doc: c"Create a raw-DEFLATE decompressor.".as_ptr() as *mut c_char,
    },
    PyMethodDef {
    ...
    PyMethodDef {
        ...
        ml_flags: METH_FASTCALL,
        ml_doc: c"Return compressed bytes not yet consumed.".as_ptr() as *mut c_char,
    },
    PyMethodDef {
