...
def apply_patch(source: Path, patch: Path, *, strip: int = 1) -> None:
    ...
    dry_run = subprocess.run(
        ["patch", f"-p{strip}", "--fuzz=0", "--dry-run", "--forward", "-i", str(patch)],
    ...
    result = subprocess.run(
        ["patch", f"-p{strip}", "--fuzz=0", "--forward", "-i", str(patch)],
        cwd=source, capture_output=True, text=True,
    )
...
def cpython_patch_directories(root: Path, target: Target) -> tuple[Path, ...]:
    """Common CPython fixes precede patches for Fil-C's distinct C ABI."""
    common = root / "cpython"
    if target.is_filc:
        return common, common / "filc"
    return (common,)
