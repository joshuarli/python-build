...
### 2026-09-18 — pip, ensurepip, and venv are excluded from the product

- **The distribution ships no package manager and no environment-creation facility.** `pip`, `ensurepip` (including its bundled wheel at `lib/python3.14/ensurepip/_bundled/`), and `venv` are removed from the installed tree on every family. `python -m pip`, `python -m ensurepip`, and `python -m venv` must all fail, and validation asserts their absence the same way it asserts the Tcl/Tk absence.
- This **supersedes** Section 6's "install pip offline from a pinned wheel" policy and Section 2.2's requirement that pip and virtual environments work. Both are rewritten to require the inverse.
- The consequences are deliberate, not incidental: creating an isolated environment from this distribution is the consumer's job, not a capability this artifact provides, and the artifact carries no Python package-installer code that could reach the network. Test and fixture tooling must not reintroduce a package installer into the payload — a separate, locked wheelhouse may hold wheels for building test fixtures, but nothing from it is installed into the shipped tree.
- `pip` is removed from `sources.lock.json`: an input that is never acquired, built, or shipped does not belong in a lock (Section 4).
...
The GUI closure extends past what is merely not built. `_tkinter` is absent because Tcl/Tk is never acquired, but `tkinter` is pure Python and is installed unconditionally, so it is **removed from the installed tree** like the packaging components — an installed-but-unimportable package is worse than an absent one, because a `find_spec`-style probe reports it as present. The same reasoning removes the GUI-only parts that cannot function without Tk: `idlelib` and its `idle3`/`idle3.14` launchers, and `turtle`. What remains excluded by construction rather than by removal is Tcl/Tk itself, the X11 closure, and `_tkinter`.
...
The finished installation must provide the exact interpreter version and architecture, normal extension loading, correct ABI and packaging tags, a useful standard-library inventory, private non-platform native dependencies, relocation, TLS verification, compression, databases, FFI, and usable extension-development metadata. It must **not** provide a package manager or environment creator: `pip`, `ensurepip`, and `venv` are excluded by the 2026-09-18 scope decision, and their absence is a tested requirement rather than an omission. Shared libpython and a tested embedding contract are required; the interpreter's own static-versus-shared libpython linkage is a documented implementation choice. On macOS the shared library is a `.dylib` with an install name, and the interpreter's load commands determine whether relocation works; record both.
...
The macOS split, **confirmed against the pinned reference archive's own Mach-O load commands** rather than inferred from a summary table:

| Built from locked source | Taken from the platform |
| --- | --- |
| OpenSSL 3.5.7, SQLite 3.53.1, Expat 2.8.1, libffi 3.8.0, bzip2 1.0.8, xz/liblzma 5.8.1, zstd 1.5.7, mpdecimal 4.0.0 | `libSystem`, zlib (`/usr/lib/libz.1.dylib`), libedit (`/usr/lib/libedit.3.dylib`), ncurses and panel (`/usr/lib/libncurses.5.4.dylib`, `/usr/lib/libpanel.5.4.dylib`), ndbm for the dbm backend |

The measurement corrected two assumptions that the earlier draft of this table got wrong: the reference does **not** bundle ncurses (it links the platform's 5.4), and it does **not** use the system Expat (it carries no `libexpat` load command, so Expat is statically linked from source like OpenSSL and SQLite). Verify this against the artifact, not against a table: the reference's non-`@rpath` dependencies are the authoritative answer for which libraries the platform is expected to supply.

Three consequences are deliberate and recorded rather than discovered late: the macOS payload carries no bundled `libuuid` and no Berkeley DB, because `_uuid` and `dbm.ndbm` use platform facilities, which also keeps an AGPL-licensed component out of the distribution; and the Tcl/Tk load commands in the reference (AppKit, Cocoa, Carbon, and the rest of that framework closure) are excluded with Tcl/Tk itself. Do not silently replace libedit with GNU readline, drop a backend, or swap zstd support for an import skip.
...
**Ship no package manager and no environment creator.** `pip`, `ensurepip` (with its bundled wheel), and `venv` are removed from the installed tree, and their absence is asserted by validation — the same enforcement style as the Tcl/Tk exclusion. Nothing in the build may reintroduce pip: installing a wheel into the shipped tree, seeding a launcher, or running ensurepip at package time are all out of scope. Creating an isolated environment is the consumer's problem, using whatever tooling they already trust.

This makes a separate concern sharper, not softer: **consumer extension development must work without a package installer in the tree.** `python3.14-config`/`sysconfig`-derived flags, a compiler, and the shipped headers must be sufficient to build and load a native extension from a plain source directory, and validation must prove that with the wheelhouse absent and no installer present.

A separate, locked wheelhouse may hold wheels used to build test fixtures, but nothing from it may be installed into the shipped tree. Do not preinstall setuptools into the base distribution just for build tests. Do not apply Homebrew's or any distribution's `EXTERNALLY-MANAGED` policy to this standalone installation; this tree is not a system Python.
...
**The profile participates in the build, so an over-tight profile silently degrades the artifact.** CPython's configure does not only compile probes, it *executes* some of them, and a denied syscall makes such a probe return "no" rather than fail. The POSIX-semaphore probe is the concrete case: denying `sem_open` defines `POSIX_SEMAPHORES_NOT_ENABLED`, which removes `SemLock` from `_multiprocessing`, so `multiprocessing.Queue` and `Pool` stop working — with no build error at all, and a sealed artifact strictly worse than an unsealed one from identical sources. Two consequences follow. The profile must permit the facilities a normal build expects (`ipc-posix-sem` among them), and validation must assert the **observable capabilities** that a skewed probe would remove, not merely that the build succeeded.

**What the macOS profile actually confines, stated exactly.** It denies the network and confines *writes* to the declared output directories plus the per-user Darwin scratch directory that the linker's cache database needs. It does **not** confine reads: a compiler driver has to read the SDK, the toolchain, and the system frameworks, and narrowing that further buys little against this threat model. Every report that describes the run must say this rather than implying the container boundary the Linux targets have. The environment is built from an allowlist rather than inherited and pruned, so an ambient variable cannot redirect a search path into the build.
...
- Confirm the intentional exclusion of `pip`, `ensurepip`, and `venv`: the modules, the bundled wheel, and any launcher must all be absent, and `python -m pip` / `python -m venv` must fail rather than silently work. Record the reference difference — the reference distribution ships pip, so this is a real capability gap for anyone expecting it.
- Consumer extension development after relocation **without any installer present**: build and import a native extension from a plain source directory using only the shipped headers, `sysconfig`, and the compiler, and do the same for a shared-library-free ABI3 fixture.
...
Use deterministic entry order, ownership, modes, timestamps, and compressor settings. Preserve symlinks and executable permissions without marking every file executable. Select a documented source epoch and bytecode policy. Include required licenses and notices. A structured component manifest should describe what is actually shipped rather than listing every build-root tool — and must not claim a component the tree does not contain. `pip` is a locked input today but is **not** a shipped component and must not appear as one (Section 6, 2026-09-18 decision). Note explicitly which components are platform-provided rather than bundled, since macOS's split (Section 5.2) means several modules draw on system libraries, and list the intentional exclusions (Tcl/Tk, pip/ensurepip/venv) with their reference differences.
...
Complete the in-scope dependency, module, and resource inventory, including the platform-versus-bundled split of 5.2 confirmed against the reference's own load commands. Implement and test Mach-O relocation, re-signing, external native extensions **built and loaded with no installer in the tree**, embedding against the shipped shared libpython, callbacks, TLS against the system trust store in both directions, multiprocessing, databases, compression, and terminal support. Enforce both the Tcl/Tk GUI closure and the packaging exclusion (pip/ensurepip/venv) as *tested absences*, not omissions. Evaluate individual portability fixes against actual failures. Measure the deployment-floor and CPU compatibility envelope and compare the installation with the pinned reference, disclosing the no-PGO policy. Resolve material gaps before declaring the platform complete.

Two findings from this phase that the design must state rather than bury. A **relocatable** libpython derives its prefix from the running executable, so an embedder outside the install tree must set `PYTHONHOME` or live inside it — an absolute-prefix build has no such requirement, and this is the cost of the tree being movable. And the sealed profile **participates in the build**, because configure executes some probes; a denial that is invisible at build time can silently remove a capability (Section 7).
...
4. The useful standard-library inventory, extension loading/development, embedding, resources, relocation, and code-signature validity tests pass, **and the pip/ensurepip/venv exclusion is enforced by test**. Material reference differences are explained and justified, including the no-PGO optimization difference and the absent package manager. Unexplained missing core behavior blocks completion.
...
9. The frozen Linux targets' code and unit tests still pass. This milestone **did** change their behaviour deliberately, in response to defects the macOS work exposed, and each change is recorded rather than silent: `make`-style recipes now pass `CC`/`AR`/`RANLIB` on the command line (bzip2's Makefile assigns `CC=gcc`, so the Linux payload was linking a gcc-built libbz2 into a clang-built module); installed launcher scripts get a relocatable shebang (they previously pointed at the configure-time `/install/bin/python3.14`, which exists nowhere); a full dependency build now owns its prefix and scratch area (a stale `libncursesw.a` could otherwise be linked after ncurses left the dependency set); and `pip` is no longer claimed as a component. Their artifacts were **not** rebuilt or re-validated in this milestone, so their recorded evidence describes the pre-change tree, not the current one.
